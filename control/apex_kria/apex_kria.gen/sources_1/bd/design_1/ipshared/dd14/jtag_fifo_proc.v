...
(
   input  wire           CLK        ,
   input  wire [3 : 0]   C_TCK_CLOCK_RATIO, 
   input  wire           RESET      ,
   input  wire           ENABLE     ,
   output reg            DONE       ,
   ...
   input  wire [63 : 0 ] TMS_SERIAL_VECTOR ,
   output wire           TCK        ,
   output wire           TMS        ,
   output wire           TDI        ,
   input  wire           TDO        ,
      ...
      if (RESET == 1'b1) begin
         ...
         if (enable_red == 1'b1) begin
            ...
            if (SERIAL == 1'b1) begin
               bit_count <= SERIAL_S_AXIS_DATA_LENGTH-1; 
            end else begin
   ...
   always @(posedge CLK) begin
      if (RESET == 1'b1) begin
         ...
         if (enable_red == 1'b1) begin
            ...
            tms_serial_output  <= TMS_SERIAL_VECTOR;
            tdo_capture <= 0;
         end else if (tck_en == 1'b1) begin
            if (tck_pulse == 1'b1) begin
               tck_i <= ~tck_i;
               if (state == TCKH) begin
                  if (SERIAL == 1'b1) begin
                     tms_serial_output  <= {1'b0, tms_serial_output[63:1]};
                  end else begin
...
assign TMS = (SERIAL == 1'b1) ? tms_serial_output[0] : tms_output[0];
//      OBUFT u0_OBUFT (
//          .O (TMS) ,
//          .I (1'b0) ,
//          .T (~tms_output[0])
